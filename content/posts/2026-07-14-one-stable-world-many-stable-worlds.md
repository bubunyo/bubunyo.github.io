...
Some context on what "the system" means here. We run a microservice architecture deployed to Kubernetes, with multiple teams owning several services, each with a single delegated task. We also depend on a Postgres database, Kafka, a secrets vault and most things used by modern-day cloud-native platforms. Let's call the whole system a world. Services are also grouped into sets of major flows. One such critical set we care about is the [straight-through order processing](https://en.wikipedia.org/wiki/Straight-through_processing) flow: if an order is placed, which services are involved end to end, of which there are a few. Everything that happens between you clicking buy and us confirming the stock is yours. If that path breaks, the business feels it immediately.
...
Think about the traditional economics of a project like this. An isolated environment tool touches Kubernetes, databases, secrets infrastructure, and Kafka internals. Under the old model that is a multi-quarter effort. Design documents, reviews, prototypes, more reviews, and months of iteration before anyone can touch a working version. Projects like this rarely die because they are bad ideas. They die because the upfront cost never clears the bar against everything else competing for the same engineers. That is why demand can sit unanswered for years.

AI changed the shape of that cost curve. I sat down with an AI assistant and iterated. Try an approach to TLS termination, watch it fail, understand why, try the next one, all within an afternoon instead of a sprint. The expensive part of building software has always been the iteration loop: the time between having an idea and learning whether it survives contact with reality. AI compresses that loop brutally. A few weeks of this, and I had a working system spanning four domains that would each have been a project on their own.

The interesting consequence is not that one tool got built faster. It is that a whole class of projects that used to be economically unjustifiable is now viable. If there is an internal tool your team has wanted for years, the math may have changed. It is worth rerunning the numbers.
...
So I flipped it on its head. Share all the infrastructure, and make every layer multi-tenant. One Postgres. One Kafka. One secrets endpoint. Each world gets a partitioned slice.
...
I set this up as a transformation pipeline. Source manifests go in one end, an ordered series of transformations runs over them, and stripped-down dev manifests come out the other. That decision paid for itself. A pipeline is testable, each transformation is composable, and when a later problem needs something injected into every manifest, the injection is just one more stage. It shows up again twice below.
...
The trick was a three-layer fake. First, DNS. Inside the local cluster, we rewrite DNS so the Secret Manager hostname resolves to a [secrets emulator I found on GitHub](https://github.com/blackwell-systems/gcp-emulator) instead of the real cloud API.
...
A full world in our setup is around 150 services. You can run that on a laptop. You will not enjoy it. So the default workflow builds a VM in Google Cloud with [Packer](https://developer.hashicorp.com/packer) and [Terraform](https://developer.hashicorp.com/terraform) , syncs the binary over, and runs the whole stack there under systemd: kind for the services, plus containerized Kafka, Postgres, the secrets emulator, and kroxy, managed with [nerdctl](https://github.com/containerd/nerdctl) as a replacement for docker compose.
...
And worlds are throwaway by design. So updating one is as simple as throwing what you currently have away and conjuring a new one, which always reflects the current state of the system. Migrating a lingering world forward is a convenience feature for later, if ever.
